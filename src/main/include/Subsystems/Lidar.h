...
typedef struct {
	...
	int basePointAngle;			// stores the angle on which scoreGroup() is based (the average angle of all the points that are equally close)
...
public:
	...
	void groupPoints();
	double angleOnCargo(double distance); // distance must be cast to a double in the function call
	...
	//Game piece placement functions
	bool getHatchPlacement();
	void checkLinesForRocketCargo();
