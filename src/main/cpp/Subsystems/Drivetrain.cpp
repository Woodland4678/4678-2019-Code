...
Drivetrain::Drivetrain() : frc::Subsystem("Drivetrain") {
    ...
    m_Path = new PathFinder(0.02,4,0.5,1,0.545);
...
void Drivetrain::resetGyroValue() {
    gyro->Reset();
}
double error = 0;
...
int Drivetrain::autoScore(int autoType) {
	...
	switch (as_m_case) {
        case  autoScoreInit:
            ...
            as_search_mode = 0;
            if(autoType == 0)
                as_move1 = Robot::manipulatorArm->moveToXY(A_SC_START_POS_X,A_SC_START_POS_Y,A_SC_START_POS_W_H,A_SC_START_POS_WAIST,ARMSPEED_MEDIUM);
...
bool Drivetrain::testPath() {
    switch(pathState) {
        case 0:
            ...
            if(m_Path->makePath()) {
                m_Path->debug();
                lPEnc = getLeftEncoder();
                rPEnc = getRightEncoder();
                pathState++;
            }
            else{
                pathState = 0;
        ...
        case 2:
            ...
            setRightPosition(rEnc);
            setLeftPosition(lEnc);
            tCnt++;
            return tdone;
            }
            break;
