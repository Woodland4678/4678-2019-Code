...
Climber::Climber() : frc::Subsystem("Climber") {
    ...
    int cnt = 0;
	while((fabs(readTalonSRXEncoder() - readABSEncoder()) > 10.0)&&(cnt < 50)) {
		stilts->SetSelectedSensorPosition(pos);
        stilts->GetSensorCollection().SetPulseWidthPosition(pos);
		cnt++;
	}
    
    m_StartPosition =  fabs(readABSEncoder() - 1050)+readTalonSRXEncoder();
...
void Climber::moveTo() {
    ...
    stilts->Set(ControlMode::Position, -27500);
}
